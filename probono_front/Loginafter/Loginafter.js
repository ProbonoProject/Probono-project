const btn2 = document.getElementById("Section__search__btn");
...
function printChoice() {
	...
	async function request() {
		const response = await fetch(
			`http://localhost:8091/api/v1/persons/talent?talentId=${choice}`,
			{
				method: "GET",
			}
		);
...
btn2.addEventListener("click", printChoice);
btn2.addEventListener("click", choiceNum);
